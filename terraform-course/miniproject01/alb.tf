...
resource "aws_lb_target_group" "project01_alb_target_group" {
  name        = "project01_alb_target_group"
  port        = 443
  protocol    = "HTTPS"
...
resource "aws_lb_target_group_attachment" "ec2_target_attachment" {
  ...
  target_group_arn = aws_lb_target_group.project01_alb_target_group.arn
